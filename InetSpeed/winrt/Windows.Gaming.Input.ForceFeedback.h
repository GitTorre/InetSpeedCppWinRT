...
{
    check_hresult(shim()->abi_SetParameters(get(direction), positiveCoefficient, negativeCoefficient, maxPositiveMagnitude, maxNegativeMagnitude, deadZone, bias));
}
...
template <typename D> Windows::Gaming::Input::ForceFeedback::ConditionForceEffect impl_IConditionForceEffectFactory<D>::CreateInstance(Windows::Gaming::Input::ForceFeedback::ConditionForceEffectKind effectKind) const
{
    ...
    check_hresult(shim()->abi_CreateInstance(effectKind, put(value)));
    return value;
}
...
{
    check_hresult(shim()->abi_SetParameters(get(vector), get(duration)));
}
...
{
    check_hresult(shim()->abi_SetParametersWithEnvelope(get(vector), attackGain, sustainGain, releaseGain, get(startDelay), get(attackDuration), get(sustainDuration), get(releaseDuration), repeatCount));
}
...
{
    check_hresult(shim()->abi_SetParameters(get(vector), frequency, phase, bias, get(duration)));
}
...
{
    check_hresult(shim()->abi_SetParametersWithEnvelope(get(vector), frequency, phase, bias, attackGain, sustainGain, releaseGain, get(startDelay), get(attackDuration), get(sustainDuration), get(releaseDuration), repeatCount));
}
...
{
    check_hresult(shim()->abi_SetParameters(get(startVector), get(endVector), get(duration)));
}
...
{
    check_hresult(shim()->abi_SetParametersWithEnvelope(get(startVector), get(endVector), attackGain, sustainGain, releaseGain, get(startDelay), get(attackDuration), get(sustainDuration), get(releaseDuration), repeatCount));
}

template <typename D> bool impl_IForceFeedbackMotor<D>::AreEffectsPaused() const
{
    bool value {};
    check_hresult(shim()->get_AreEffectsPaused(&value));
    return value;
}
...
template <typename D> Windows::Gaming::Input::ForceFeedback::ForceFeedbackEffectAxes impl_IForceFeedbackMotor<D>::SupportedAxes() const
{
    ...
    check_hresult(shim()->get_SupportedAxes(&value));
    return value;
}
...
template <typename D> Windows::Foundation::IAsyncOperation<bool> impl_IForceFeedbackMotor<D>::TryDisableAsync() const
{
    ...
    check_hresult(shim()->abi_TryDisableAsync(put(asyncOperation)));
    return asyncOperation;
}
...
{
    Windows::Foundation::IAsyncOperation<bool> asyncOperation;
    check_hresult(shim()->abi_TryEnableAsync(put(asyncOperation)));
    return asyncOperation;
}
...
{
    Windows::Foundation::IAsyncOperation<bool> asyncOperation;
    check_hresult(shim()->abi_TryResetAsync(put(asyncOperation)));
    return asyncOperation;
}
...
{
    Windows::Foundation::IAsyncOperation<bool> asyncOperation;
    check_hresult(shim()->abi_TryUnloadEffectAsync(get(effect), put(asyncOperation)));
    return asyncOperation;
}
