...
{
    bool value {};
    check_hresult(shim()->get_IsIntegrated(&value));
    return value;
}
...
template <typename D> Windows::Foundation::Rect impl_IPointerDevice<D>::PhysicalDeviceRect() const
{
    ...
    check_hresult(shim()->get_PhysicalDeviceRect(put(value)));
    return value;
}
...
{
    Windows::Foundation::Rect value {};
    check_hresult(shim()->get_ScreenRect(put(value)));
    return value;
}
...
{
    uint32_t value {};
    check_hresult(shim()->get_MaxPointersWithZDistance(&value));
    return value;
}
...
template <typename D> Windows::Devices::Input::MouseDelta impl_IMouseEventArgs<D>::MouseDelta() const
{
    ...
    check_hresult(shim()->get_MouseDelta(put(value)));
    return value;
}
