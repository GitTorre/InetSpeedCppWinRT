...
{
    hstring value;
    check_hresult(shim()->get_OptionId(put(value)));
    return value;
}
...
{
    bool succeeded {};
    check_hresult(shim()->abi_TrySetValue(get(value), &succeeded));
    return succeeded;
}
...
{
    uint32_t value {};
    check_hresult(shim()->get_MaxCharacters(&value));
    return value;
}
...
{
    check_hresult(shim()->put_MaxCharacters(value));
}
...
{
    check_hresult(shim()->put_ItemDisplayName(get(value)));
}

template <typename D> hstring impl_IPrintCustomItemDetails<D>::ItemDisplayName() const
{
    hstring value;
    check_hresult(shim()->get_ItemDisplayName(put(value)));
    return value;
}
...
{
    check_hresult(shim()->abi_AddItem(get(itemId), get(displayName)));
}
...
template <typename D> Windows::Graphics::Printing::OptionDetails::PrintCustomItemListOptionDetails impl_IPrintTaskOptionDetails<D>::CreateItemListOption(hstring_ref optionId, hstring_ref displayName) const
{
    ...
    check_hresult(shim()->abi_CreateItemListOption(get(optionId), get(displayName), put(itemListOption)));
...
template <typename D> Windows::Graphics::Printing::OptionDetails::PrintCustomTextOptionDetails impl_IPrintTaskOptionDetails<D>::CreateTextOption(hstring_ref optionId, hstring_ref displayName) const
{
    ...
    check_hresult(shim()->abi_CreateTextOption(get(optionId), get(displayName), put(textOption)));
    return textOption;
}
...
template <typename D> event_token impl_IPrintTaskOptionDetails<D>::OptionChanged(const Windows::Foundation::TypedEventHandler<Windows::Graphics::Printing::OptionDetails::PrintTaskOptionDetails, Windows::Graphics::Printing::OptionDetails::PrintTaskOptionChangedEventArgs> & eventHandler) const
{
    ...
    check_hresult(shim()->add_OptionChanged(get(eventHandler), &eventCookie));
    return eventCookie;
}
...
{
    check_hresult(shim()->remove_OptionChanged(eventCookie));
}
...
{
    event_token eventCookie {};
    check_hresult(shim()->add_BeginValidation(get(eventHandler), &eventCookie));
    return eventCookie;
}
...
{
    check_hresult(shim()->remove_BeginValidation(eventCookie));
}
...
template <typename D> Windows::Graphics::Printing::OptionDetails::PrintTaskOptionDetails impl_IPrintTaskOptionDetailsStatic<D>::GetFromPrintTaskOptions(const Windows::Graphics::Printing::PrintTaskOptions & printTaskOptions) const
{
    ...
    check_hresult(shim()->abi_GetFromPrintTaskOptions(get(printTaskOptions), put(printTaskOptionDetails)));
