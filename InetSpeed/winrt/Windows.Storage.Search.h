...
{
    bool value {};
    check_hresult(shim()->abi_AreQueryOptionsSupported(get(queryOptions), &value));
    return value;
}
...
{
    bool value {};
    check_hresult(shim()->abi_IsCommonFolderQuerySupported(query, &value));
    return value;
}
...
{
    bool value {};
    check_hresult(shim()->abi_IsCommonFileQuerySupported(query, &value));
    return value;
}
...
template <typename D> Windows::Storage::Search::StorageItemQueryResult impl_IStorageLibraryContentChangedTriggerDetails<D>::CreateModifiedSinceQuery(const Windows::Foundation::DateTime & lastQueryTime) const
{
    ...
    check_hresult(shim()->abi_CreateModifiedSinceQuery(get(lastQueryTime), put(result)));
    return result;
}
